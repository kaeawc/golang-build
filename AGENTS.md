...
- `internal/tokens/` — JWT-compatible signed tokens. `NewHMAC256(secret, opts...)` issues and verifies HS256 tokens. Embed `StandardClaims` (sub/iss/aud/iat/exp/nbf/jti) into your own claims struct. `Verify` enforces signature, alg (`HS256` only — defends against `alg=none` and substitution), exp/nbf with optional `WithLeeway` for clock skew, and reads time from `clock.Clock` so expiration is testable. Sentinel errors `ErrMalformed`, `ErrSignature`, `ErrAlgorithm`, `ErrExpired`, `ErrNotYetValid` for handler error mapping. `AlwaysValid(claims)` and `AlwaysInvalid(err)` produce a `Fake` `IssuerVerifier` for tests with `SignCount` / `VerifyCount` introspection. RSA/ECDSA intentionally out of scope.
